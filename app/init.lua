...
local function create_schema()
	...
	if box.space.folder_props == nil then
		local folder_props = box.schema.create_space("folder_props")
		folder_props:create_index("primary", {
			type = "tree",
			unique = true,
			parts = {1, "str", 2, "str", 3, "str"}
		})
	...
	if box.space.user_props == nil then
		local user_props = box.schema.create_space("user_props")
		user_props:create_index("primary", {
			type = "tree",
			unique = true,
			parts = {1, "str", 2, "str"}
		})
...
local function create_admin()
	...
	if admin == nil then
		...
		space.create_space("admin", "admin", nil)
		space.create_space("examples", "admin", nil)
...
function create_rschema()
	global_cfg.tables.rdocs = {
		name = "rdocs",
		fields = {
			{
				name = "doc_id",
	...
	for k, table_ref in pairs(global_cfg.tables) do
		utils.create_table(table_ref)
	end

	if box.space.rtags == nil then
		local rtags = box.schema.create_space("rtags")
		rtags:create_index("primary", {
			type = "hash",
			unique = true,
			parts = {1, "str"}
		})
		rtags:create_index("by_tag", {
			type = "tree",
			unique = true,
			parts = {2, "str", 3, "num"}
		})
