...
function api_download_table_rows(req, session, headers)
    local doc_id = req:stash("first")
    local ok, result = rules.download_table_rows(
    	doc_id,
    	session.user_id,
    	session.roles
    )
    if ok then
        return make_text_response(200, headers, result)
    else
        return result_from_message(headers, result)
    end
end
...
function api_get_folder_props(req, session, headers)
    local space_id = req:stash("first")
    local folder_id = req:stash("second")
    local ok, result = space.get_folder_props(
    	space_id,
    	folder_id,
    	session.user_id,
    	session.roles
...
function api_get_gentoken(req, session, headers)
    local space_id = req:stash("first")
    local ok, result = space.get_create_gentoken(
    	space_id,
    	session.user_id,
    	session.roles
    )
...
function api_get_module(req, session, headers)
    local space_id = req:stash("first")
    local folder_id = req:stash("second")
    local ok, result = space.get_module(
    	space_id,
    	folder_id,
    	session.user_id,
    	session.roles
...
function api_get_prog_modules(req, session, headers)
    local space_id = req:stash("first")
    local ok, result = space.get_prog_modules(
    	space_id,
    	session.user_id,
...
function api_lite(api_method, method, must_logon, must_admin, handler)
    local key = api_method .. "/" .. method
    local handler_info = {
    	must_logon = must_logon,
    	must_admin = must_admin,
...
function api_logon(req, session, headers)
    ...
    if ((data) and (data.user)) and (data.password) then
        local ok, msg, user_id, email, new_session, udata =
        vud.logon(
        	session.session_id,
        	data.user,
        	data.password
        ...
        if ok then
            ...
            if udata.system then
                data.authorization = new_session
            else
                set_session_cookie(
                	headers,
                	new_session
...
function api_rules(req, session, headers)
    local data = req:json()
    local ok, result = rules.api(
    	data,
    	session.user_id,
    	session.roles
    )
...
function api_set_folder_props(req, session, headers)
    local space_id = req:stash("first")
    local folder_id = req:stash("second")
    local data = req:json()
    local msg = space.set_folder_props(
    	space_id,
    	folder_id,
    	data,
    	session.user_id,
...
function api_upload_table_rows(req, session, headers)
    local doc_id = req:stash("first")
    local body = req:read()
    local ok, result = rules.upload_table_rows(
    	doc_id,
    	body,
    	session.user_id,
    	session.roles
...
function copy_examples(user_id)
    local roles = {admin=true}
...
function create_space(space_id, user_id)
    local folder_id = rules.create_root_folder(
    	space_id,
    	user_id
    )
    space.create_space(
...
function delete_user(user_id)
    ...
    local user = vud.get_user(user_id)
    if user then
        rules.delete_recent_by_user(user_id)
...
function get_access_for_user(user_id, output)
    ...
    for space_id, group in pairs(by_space) do
        ...
        if sdata.root_id then
            table.insert(
            	projects,
                    {
                      space_id=space_id,
                      root_id=sdata.root_id,
                      access=access
                    }
            )
        end
...
function get_session_id(req)
    local session_id
    session_id = req:header("authorization")
    if session_id then
...
function handle_cookie_kernel(req, headers)
    local ip = get_client_ip(req)
    local referer = req:header("referer")
    local path = req:path()
    local session_id = get_session_id(req)
...
function handle_template(req, page, url_language, caching_allowed, cinfo_maker)
    ...
    if error_message then
        ...
        local vars = {
        	...
        	admin = session.roles.admin or false,
        	debug = session.debug,
...
function make_text_response(status, headers, data)
    headers["content-type"] =
...
function start()
    ...
    if global_cfg.rul then
        route("/robots.txt", handle("robots-rul.txt"))
        route("/favicon.ico", handle("favicon-rul.ico"))
        static_simple("", "rul.html")
        static_simple("/edit", "edit.html")
        static_simple("/edit/:doc_id", "edit.html")
        static_simple("/edit/:doc_id/:item_id", "edit.html")
        static_simple("/play/:doc_id", "edit.html")
        api("rules", "POST", false, false, api_rules)
        api("get_tags", "POST", false, false, api_rules)
        api("get_changes", "POST", false, false, api_rules)
        api("trows", "POST", true, false, api_upload_table_rows)
        api("trows", "GET", false, false, api_download_table_rows)
    else
    ...
    api_lite("tag", "GET", false, false, api_get_tag)
...
function static_simple(path, filename)
