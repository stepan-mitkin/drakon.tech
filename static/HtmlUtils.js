...
function updateMoverPos(movable) {
	var div = movable.div
...
function onMoverMouseDown(evt, movable) {
	var pos = ns.leftTop(movable.div)
	movable.pos = pos
	movable.mx = evt.clientX
	movable.my = evt.clientY
	movable.drag = true
...
function onMoverMouseMove(evt, movable) {
	if (!movable.drag) {
		return
	}
	var dx = evt.clientX - movable.mx
	var dy = evt.clientY - movable.my
	movable.mx = evt.clientX
	movable.my = evt.clientY
	movable.pos.x += dx
	movable.pos.y += dy
...
function onMoverMouseUp(evt, movable) {
	if (!movable.drag) {
		return
	}
	movable.drag = false
	updateMoverPos(movable)
	ns.preventDefaultHandling(evt)
}
...
ns.setUpMovable = function(div, height) {
	...
	// if (!base) {
	// 	throw new Error("Cannot set up movable: no dialog")
	// }
	var base = div
		
	var mover = make(base, "div")
	mover.style.zIndex = 300
	// mover.style.background = "orange"
	// mover.style.opacity = 0.3
	mover.oncontextmenu = function() { return false }
	movable.mover = mover
	
	registerEvent(mover, "mousedown", function(evt) {onMoverMouseDown(evt, movable) })
	registerEvent(mover, "mousemove", function(evt) {onMoverMouseMove(evt, movable) })
	registerEvent(mover, "mouseup", function(evt) {onMoverMouseUp(evt, movable) })
	
	registerEvent(mover, "touchstart", function(evt) {
		ns.preventDefaultHandling(evt)
		var evt2 = ns.touchToMouse(evt)
		onMoverMouseDown(evt2, movable)
	})
	registerEvent(mover, "touchmove",  function(evt) {
		ns.preventDefaultHandling(evt)
		var evt2 = ns.touchToMouse(evt)
		onMoverMouseMove(evt2, movable)
	})
	registerEvent(mover, "touchend", function(evt) {onMoverMouseUp(evt, movable) })	
	registerEvent(mover, "touchcancel", function(evt) {onMoverMouseUp(evt, movable) })
